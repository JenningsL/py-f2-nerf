...
__global__ void RayMarchKernel(int n_rays, float sample_l, bool scale_by_dis,
                               ...
                               float* sampled_dists, float* sampled_dists_real, float* sampled_ts, int* sampled_oct_idx,
                               float* first_oct_dis) {
  int ray_idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (ray_idx >= n_rays) {
    return;
  ...
  if (FILL) {
    ...
    sampled_dists_real = sampled_dists_real + pts_idx;
  ...
  while (pts_ptr < max_n_samples && oct_ptr < n_oct_nodes) {
    ...
    if (FILL && !the_first_pts) {
      ...
      sampled_dists_real[pts_ptr] = exp_march_step;
...
SampleResultFlex PersSampler::GetSamples(const Tensor& rays_o_raw, const Tensor& rays_d_raw, const Tensor& bounds_raw) {
  ...
  Tensor sampled_dists_real = torch::empty({ n_all_pts }, CUDAFloat);
  ...
  RayMarchKernel<true><<<grid_dim, block_dim>>>(
      ...
      sampled_dists.data_ptr<float>(), sampled_dists_real.data_ptr<float>(), sampled_t.data_ptr<float>(),
